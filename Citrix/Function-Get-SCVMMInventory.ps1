...
.EXAMPLE
   Get-SCVMMInventory -SCVMMServer scvmm01 -ClusterName hvcluster01
   Get a list of all the vm's on hvcluster01 with their cpu, memory
   and disk.
.EXAMPLE
   Get-SCVMMInventory -SCVMMServer scvmm01 -ClusterName hvcluster01 -Summary
   Get a total list of VM's, cpu, memory, and disk for cluster hvcluster01.
.EXAMPLE
   Get-SCVMMInventory -SCVMMServer scvmm01 -Summary
   Get a total list of VM's, cpu, memory, and disk for all clusters.
.EXAMPLE
   Get-SCVMMInventory -SCVMMServer scvmm01 -ClusterName hvcluster01 | Sort-Object -Property Name | Out-Gridview
   Get a list of all the vm's on hvcluster01 with their cpu, memory
   and disk sorted by VM name, displayed in gridview. 
.EXAMPLE
   Get-SCVMMInventory -SCVMMServer scvmm01 -Summary | Out-GridView
   Get a total list of VM's, cpu, memory, and disk for all clusters displayed in gridview.
.LINK
   https://gallery.technet.microsoft.com/scriptcenter/Get-SCVMMInventory-Get-b0f92a99
#>
