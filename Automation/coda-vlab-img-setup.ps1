...
#command to call script: Set-ExecutionPolicy Bypass -Scope Process -Force; & '\\gt-repo.ad.gatech.edu\configs$\vlab source files\vlab-image-setup.ps1'
...
if ($userinput -eq 'y')
{
    ...
    $userinput = $(Write-Host "Select Nvidia Driver Version: 8 or 11. Any other option will exit nvidia installer. " -ForegroundColor Green -NoNewline; Read-Host) 
    
    Switch ($userinput) 
    { 
        8
        {
            Write-Host -ForegroundColor Yellow "Installing Nvidia Driver 8"
        ...
        11 
        {
            Write-Host -ForegroundColor Yellow "Installing Nvidia Driver 11"
            Start-Process -FilePath "$localpath\nvid\11.3\DisplayDriver\452.77\Win10_64\International\setup.exe" -ArgumentList $install_args -wait } 
        
        Default 
        {
            Write-Host "Canceling"; $PublishSettings=$false
        }
    }
...
if ( $userinput -eq 'y')
{
    ...
    Start-Process -Wait -FilePath "$localpath\$CitrixVDA" -ArgumentList '/controllers "coda-xd7-srv1.ad.gatech.edu coda-xd7-srv2.ad.gatech.edu" /noreboot /quiet /virtualmachine /enable_hdx_ports /enable_hdx_udp_ports /includeadditional "Citrix Supportability Tools","Machine Identity Service","Citrix User Profile Manager","Citrix User Profile Manager WMI Plugin" /exclude "Workspace Environment Management","Citrix Files for Outlook","Citrix Files for Windows","Citrix Personalization for App-V - VDA","Personal vDisk" /components vda /mastermcsimage' 
